...
- **GitHub Actions** — a scheduled (or on-demand, via a button) workflow that
  regenerates the report, publishes it to GitHub Pages, and emails it to you.
...
| `generate_report.py` | Automation entry point: fetch → write `docs/index.html` → email (if configured). |
| `email_utils.py` | Gmail SMTP sending, credentials from environment variables only. |
...
Writes `docs/index.html`. If `GMAIL_ADDRESS` / `GMAIL_APP_PASSWORD` /
`REPORT_RECIPIENT` aren't all set in your environment, it logs that and skips
the email step rather than failing — safe to run locally without any Gmail
setup.

## Setting up the automated (email + GitHub Pages) version
...
2. Create an App Password: https://myaccount.google.com/apppasswords
   (App: "Mail", any device name) — Google gives you a 16-character password.
   This is **not** your normal Gmail password, and it's the only credential
   this project ever uses to send mail.
...
| `REPORT_RECIPIENT` | The address that should receive the report (can be the same as `GMAIL_ADDRESS`) |

Credentials are **never** hardcoded anywhere in this repo — only read from
environment variables (locally) or GitHub secrets (in Actions).
...
Each run regenerates `docs/index.html`, commits it back to the repo only if
it changed, and emails the same report to `REPORT_RECIPIENT`.
